import contentClient, {previewClient} from "./client";
...
export class Delivery {
    ...
    async endpoints(contentType, preview = false) {
        try {
    ...
    async entriesById(items) {
        try {
            if(!this.client()) throw "No client loaded";
            if(!Array.isArray(items)) throw "No list of IDs provided";
    
            const pEntries = items.map(async(item) => {
                let id = item?.sys?.id || null
                if(!id) return null;
                
                return await contentClient.getEntry(id);
            })

            const entries = await Promise.all(pEntries);

            return entries.filter(i => typeof i?.fields === 'object');
        } catch (e) {
            return this.error(e);
        }
    ...
    async homepage(contentType = 'homepage', preview = false) {
        try {
            if(!this.client()) throw "No client loaded";
            if (!contentType) throw "No content type specified";
    ...
    async getPage(slug, contentType = 'page', preview = false) {
        try {
            // let pages = null;
            // if(preview) {
            ...
            //     })
            // } else {
            //     pages = await contentClient.getEntries({
            //         content_type: contentType,
            //         'fields.slug': slug
            //     })
            // }
